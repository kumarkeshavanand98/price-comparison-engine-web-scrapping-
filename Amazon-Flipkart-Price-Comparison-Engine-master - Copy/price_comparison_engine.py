...
class Price_compare:
    ...
    def __init__(self, master):
        ...
        self.var_ebay = tkinter.StringVar()
        self.var_flipkart = tkinter.StringVar()
        self.var_amzn = tkinter.StringVar()

        label = tkinter.Label(master, text='Enter the product')
        label.grid(row=0, column=0,padx=(30,10),pady=30)

        entry = tkinter.Entry(master, textvariable=self.var)
        entry.grid(row=0, column=1)

        button_find = tkinter.Button(master, text='Find', bd=4, command=self.find)
        button_find.grid(row=1, column=1, sticky=tkinter.W, pady=8)
    ...
    def find(self):
        ...
        self.n = 1
        self.key = ""
        self.title_flip_var = tkinter.StringVar()
        self.title_amzn_var = tkinter.StringVar()
        self.variable_amzn = tkinter.StringVar()
        self.variable_flip = tkinter.StringVar()
        ...
        label_title_flip = tkinter.Label(self.window, text='Flipkart Title:')
        label_title_flip.grid(row=0, column=0, sticky=tkinter.W)

        label_flipkart = tkinter.Label(self.window, text='Flipkart price (Rs):')
        label_flipkart.grid(row=1, column=0, sticky=tkinter.W)

        entry_flipkart = tkinter.Entry(self.window, textvariable=self.var_flipkart)
        entry_flipkart.grid(row=1, column=1, sticky=tkinter.W)

        label_title_amzn = tkinter.Label(self.window, text='Amazon Title:')
        label_title_amzn.grid(row=3, column=0, sticky=tkinter.W)

        label_amzn = tkinter.Label(self.window, text='Amazon price (Rs):')
        label_amzn.grid(row=4, column=0, sticky=tkinter.W)

        entry_amzn = tkinter.Entry(self.window, textvariable=self.var_amzn)
        entry_amzn.grid(row=4, column=1, sticky=tkinter.W)
        ...
        option_amzn = tkinter.OptionMenu(self.window, self.variable_amzn, *self.matches_amzn)
        option_amzn.grid(row=3, column=1, sticky=tkinter.W)

        lab_amz = tkinter.Label(self.window, text='Not this? Try out suggestions by clicking on the title')
        lab_amz.grid(row=3, column=2, padx=4)

        option_flip = tkinter.OptionMenu(self.window, self.variable_flip, *self.matches_flip)
        option_flip.grid(row=0, column=1, sticky=tkinter.W)

        lab_flip = tkinter.Label(self.window, text='Not this? Try out suggestions by clicking on the title')
        lab_flip.grid(row=0, column=2, padx=4)

        button_search = tkinter.Button(self.window, text='Search', command=self.search, bd=4)
        button_search.grid(row=2, column=2, sticky=tkinter.E, padx=10, pady=4)

        button_amzn_visit = tkinter.Button(self.window, text='Visit Site', command=self.visit_amzn, bd=4)
        button_amzn_visit.grid(row=4, column=2, sticky=tkinter.W)

        button_flip_visit = tkinter.Button(self.window, text='Visit Site', command=self.visit_flip, bd=4)
        button_flip_visit.grid(row=1, column=2, sticky=tkinter.W)
    ...
    def price_flipkart(self, key):
        ...
        self.opt_title_flip = tkinter.StringVar()
    ...
    def price_amzn(self, key):
        ...
        self.opt_title = tkinter.StringVar()
